# คู่มือผู้ดูแล — Advanced workflow
...
> นักเรียนเริ่มที่ [README](../README.md) และ `python3 student.py preview` คู่มือนี้สำหรับ scrape, review, export และการเปิดคลังเดิมแบบเลือกใช้ ขั้นตอน demo ด้านล่างรวม legacy เพื่อการ audit โดยเฉพาะ
...
Only approved original packs get ZIP download links. Drafts stay visibly draft; competitor references remain study-only. Serve only the build output. Do not deploy the repository root. `vercel.json` builds the editable `student/` project into `dist/`; it does not include the legacy archive. Deploy a curated approved gallery separately with the build command above. A private GitHub repo does not make a Vercel site private. The static MVP has no student login, per-student data isolation or durable favorites.
